...
// behaviour, not a Sightr bug and not a config knob.
//
 ...
 * tree, and a herd can hold panes from several (collie#92). The roots are searched IN ORDER and the
...
export class ClaudeTranscriptSource implements TranscriptSource {
   ...
   *    over a directory of ~40 logs. History still follows on every resolve. The snapshot poll does
   *    not: its runningCommand refresh reuses a cached path and only stats size/mtime, so this
   *    scan is not on the 1.5 s loop.
   *
