...
// Every row carries its own `id`, so there is nothing to synthesise for paging.
...
describe("parsePiTranscript", () => {
  ...
  test("a toolCall summarises from its arguments OBJECT (no JSON string)", () => {
    const entries = parsePiTranscript(
      row("a", {
        role: "assistant",
        content: [
...
describe("PiTranscriptSource — path refs are confined to the root", () => {
  ...
  async function fixture() {
    const created = join(tmpdir(), `sightr-pi-${Math.floor(performance.now() * 1000)}`);
...
// CLAUDE_CONFIG_DIR raised for Claude (collie#92). pi is the interesting one because its ref is
...
describe("PiTranscriptSource — several sessions roots", () => {
  ...
  async function fixture() {
    const created = join(tmpdir(), `sightr-pi-roots-${Math.floor(performance.now() * 1000)}`);
    await mkdir(created, { recursive: true });
    const base = await realpath(created);
