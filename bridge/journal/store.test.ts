...
describe("TranscriptStore", () => {
  ...
  test("CACHE_MAX is 16", () => {
    expect(CACHE_MAX).toBe(16);
  });

  test("two polls inside the TTL cause one refresh", async () => {
    const { adapter, calls } = fakeAdapter(["u1"]);
    let now = 1000;
    const store = new TranscriptStore(() => now);
    store.scheduleRunningCommandRefresh(adapter, REF);
    store.scheduleRunningCommandRefresh(adapter, REF); // coalesce onto the in-flight refresh
    await store.flushRunningCommandRefresh(adapter, REF);
    expect(calls.resolve).toBe(1);
    expect(calls.load).toBe(1);
    expect(calls.stat).toBe(1);
    await store.runningCommand(adapter, REF); // still inside the TTL
    expect(calls.resolve).toBe(1);
    expect(calls.load).toBe(1);
    expect(calls.stat).toBe(1);
  });

  test("an unchanged (size, mtimeMs) refresh performs no load", async () => {
    const { adapter, calls } = fakeAdapter(["u1"]);
    let now = 1000;
    const store = new TranscriptStore(() => now);
  ...
  test("peek is undefined until the first refresh settles", async () => {
    const { adapter } = fakeAdapter(["u1"]);
    const store = new TranscriptStore();
    expect(store.peekRunningCommand(adapter, REF)).toBeUndefined();
    store.scheduleRunningCommandRefresh(adapter, REF);
    expect(store.peekRunningCommand(adapter, REF)).toBeUndefined();
    await store.flushRunningCommandRefresh(adapter, REF);
    expect(store.peekRunningCommand(adapter, REF)).toBe(false);
  });
});
