...
import {
  ...
  createOperatorWheel,
  normalizeChord,
  validateOperatorKeys,
  validateOperatorWheel,
...
describe("normalizeChord", () => {
  test("accepts the grammar herdr verified", () => {
    ...
    // Function keys — the F-keys row's own grammar (collie#119).
...
function wheelRows(toml: string, warn: (msg: string) => void = quiet) {
  return validateOperatorWheel(Bun.TOML.parse(toml), warn);
}

describe("the gesture wheel's [[wheel]] rows", () => {
  test("three rows parse in file order, chords normalised", () => {
    const out = wheelRows(`
      [[wheel]]
      label = "One"
      keys = ["shift+tab"]
    ...
    `);
    expect(out).toEqual([
      { label: "One", keys: ["shift+Tab"] },
      { label: "Two", keys: ["Enter"] },
      { label: "Three", keys: ["ctrl+c"] },
    ]);
  });

  test("action = 'type' parses with no keys; action = 'nope' is dropped", () => {
    const out = wheelRows(`
      [[wheel]]
      label = "Direct"
      action = "type"
      ...
      action = "nope"
    `);
    expect(out).toEqual([{ label: "Direct", action: "type" }]);
  });

  test("both keys and action -> dropped; neither -> dropped", () => {
    expect(
      wheelRows(`
        [[wheel]]
        label = "Both"
  ...
  test("danger = true and danger = false are both dropped, and the warn text names the row", () => {
    const warns: string[] = [];
    const capture = (msg: string) => warns.push(msg);
    const out = wheelRows(
      `
        [[wheel]]
        label = "Dangerous"
    ...
    expect(out).toEqual([]);
    expect(warns.some((w) => w.includes("Dangerous") && w.includes("danger"))).toBe(true);
    expect(warns.some((w) => w.includes("Safe") && w.includes("danger"))).toBe(true);
  });
  ...
  test("scope -> dropped", () => {
    ...
    expect(out).toEqual([]);
    expect(warns.some((w) => w.includes("Scoped") && w.includes("scope"))).toBe(true);
  });

  test("seven rows -> six kept, the seventh warned", () => {
    const warns: string[] = [];
    const capture = (msg: string) => warns.push(msg);
    const toml = Array.from({ length: 7 }, (_, i) => `[[wheel]]\nlabel = "S${i + 1}"\nkeys = ["Enter"]`).join(
      "\n\n",
    );
    const out = wheelRows(toml, capture);
    expect(out).toHaveLength(6);
    expect(out.map((r) => r.label)).toEqual(["S1", "S2", "S3", "S4", "S5", "S6"]);
    expect(warns.some((w) => w.includes("S7") && w.includes("six"))).toBe(true);
  });

  test("no [[wheel]] table -> []", () => {
    expect(wheelRows("")).toEqual([]);
    expect(wheelRows(`[[keys]]\nlabel = "A"\nkeys = ["Enter"]`)).toEqual([]);
    expect(validateOperatorWheel(undefined, quiet)).toEqual([]);
    expect(validateOperatorWheel(null, quiet)).toEqual([]);
  });

  test("one reader test through the fake OperatorFileIo: a file with both [[keys]] and [[wheel]] read by createOperatorKeys and createOperatorWheel gives each its own rows", async () => {
    const text = `
      [[keys]]
      label = "TrayKey"
      keys = ["ctrl+k"]
      ...
      keys = ["Escape"]
    `;
    const { io } = fakeIo({ mtime: 100, text });
    const readKeys = createOperatorKeys("/cfg/keys.toml", io, quiet);
    const readWheel = createOperatorWheel("/cfg/keys.toml", io, quiet);

    const keys = await readKeys();
    const wheel = await readWheel();

    expect(keys).toMatchObject([{ label: "TrayKey", keys: ["ctrl+k"] }]);
    expect(wheel).toEqual([{ label: "WheelKey", keys: ["Escape"] }]);
  });
});
