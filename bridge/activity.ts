...
/** Disk shape: ledger key → pane id → activity. The outer key was once the herdr session name;
 *  one bridge now fronts one herdr, so everything lives under {@link ACTIVITY_KEY} — kept as a
 *  level so an existing activity.json loads unchanged. */
export type ActivityFile = Record<string, Record<string, PaneActivity>>;

/** The one outer key the bridge writes under (the default herdr session's old registry name). */
export const ACTIVITY_KEY = "default";
...
/** The window coalesces a burst of seenAt / activeAt stamps into one write, about 30× fewer writes
 *  for an open pane. Clean shutdown still flushes; a hard kill can lose up to 30 s of “you looked at
 *  this”, whose worst effect is one already-read pane re-appearing as unread after restart. */
...
export class ActivityLedger {
  ...
  constructor(
    ...
    /** Seam for tests to count writes. Production uses the real atomic rename. */
    private readonly renameFile: (from: string, to: string) => Promise<void> = rename,
  ) {
  ...
  async load(): Promise<void> {
    try {
      const parsed = coerceActivityFile(await readJsonOr(this.file, {}), this.now());
  ...
  private async write(): Promise<void> {
    try {
      await writeJsonAtomic(this.file, this.snapshot(), { rename: this.renameFile });
    } catch (err) {
