...
// Which agent lifecycle events are worth a push: this decides which *kinds* of alert ever fire. By
// default only "agent needs your input" (blocked) pushes; a "done" push is off — most people don't
// want a buzz for every completed task. Bridge-wide (not per-device), because a push fans out to
...
export class NotifyPrefsStore {
  ...
  async load(): Promise<void> {
    try {
      this.prefs = coerceNotifyPrefs(await readJsonOr(this.file, null));
    } catch {
  ...
  private async save(): Promise<void> {
    await writeJsonAtomic(this.file, this.prefs, { indent: 2 });
  }
}
