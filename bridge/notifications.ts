...
/** The herd's one notification slot: every agent alert renders into (and replaces) this tag. */
export const HERD_TAG = "sightr:herd";

/**
 * Build the {@link NotifySink} the coordinator drives. The whole herd shares one notification slot
 * (`herdTag`), so a render replaces rather than stacks.
 */
...
export class NotificationCoordinator<H = unknown> {
  ...
  private delayMs(): number {
    return typeof this.delay === "number" ? this.delay : this.delay();
  }
  ...
  /**
   * Tear down every notification: cancel every pending timer and retract everything outstanding,
   * closing the herd slot. Called at bridge shutdown so alerts never linger on the lock screen with
   * no live bridge behind them.
   */
  clearAll(): void {
  ...
  private summarize(renotify: boolean): HerdSummary {
    ...
    if (entries.length === 1) {
      ...
      return {
        title: `${a.agentName ?? a.agent} ${verb}`,
    ...
    return { title, body: alerts.map((a) => a.agentName ?? a.agent).join(", "), renotify };
  }
