...
export interface OperatorKeyRow extends Wire.OperatorKeyRow { danger: boolean; }
...
export const MAX_WHEEL_SLICES = 6;
interface WheelDocument {
  wheel?: unknown;
}

export function validateOperatorWheel(
  doc: WheelDocument | null | undefined,
  warn = defaultWarn,
): Wire.OperatorWheelRow[] {
  const rows = doc?.wheel;
  if (rows === undefined || rows === null) return [];
  if (!Array.isArray(rows)) {
    warn("wheel: `wheel` must be an array of [[wheel]] tables — ignoring the file's rows");
    return [];
  }
  const out: Wire.OperatorWheelRow[] = [];
  for (const raw of rows) {
    if (typeof raw !== "object" || raw === null || Array.isArray(raw)) {
      warn("wheel: ignoring a row that is not a [[wheel]] table");
      continue;
    }
    const row = raw as Record<string, unknown>;
    const label = typeof row.label === "string" ? row.label.trim() : "";
    if (label === "") {
      warn("wheel: ignoring a row with no label");
      continue;
    }
    if (row.danger !== undefined) {
      warn(`wheel: ignoring "${label}" — danger is not supported on the wheel; use a Keys tray preset`);
      continue;
    }
    if (row.scope !== undefined) {
      warn(`wheel: ignoring "${label}" — the wheel has no scope`);
      continue;
    }
    const hasKeys = row.keys !== undefined;
    const hasAction = row.action !== undefined;
    if ((hasKeys && hasAction) || (!hasKeys && !hasAction)) {
      warn(`wheel: ignoring "${label}" — give it keys or action = "type", not both`);
      continue;
    }
    let parsed: Wire.OperatorWheelRow | null = null;
    if (hasAction) {
      if (row.action !== "type") {
        warn(`wheel: ignoring "${label}" — action must be "type"`);
        continue;
      }
      parsed = { label, action: "type" };
    } else {
      if (!Array.isArray(row.keys) || row.keys.length === 0) {
        warn(`wheel: ignoring "${label}" — keys must be a non-empty array of chords`);
        continue;
      }
      const chords: string[] = [];
      let badChord = false;
      for (const entry of row.keys) {
        const chord = typeof entry === "string" ? normalizeChord(entry) : null;
        if (chord === null) {
          warn(`wheel: ignoring "${label}" — ${JSON.stringify(entry)} is not a key herdr accepts`);
          badChord = true;
          break;
        }
        chords.push(chord);
      }
      if (badChord) continue;
      parsed = { label, keys: chords };
    }

    if (out.length >= MAX_WHEEL_SLICES) {
      warn(`wheel: ignoring "${label}" — the wheel holds at most six slices`);
      continue;
    }
    out.push(parsed);
  }
...
export function createOperatorWheel(
  path: string,
  io: OperatorFileIo = diskIo,
  warn = defaultWarn,
): () => Promise<Wire.OperatorWheelRow[]> {
  return createOperatorFileReader(path, validateOperatorWheel, io, warn);
}
