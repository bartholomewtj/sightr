...
import { isReservedAuthPath } from "./static-assets.ts";
...
// staring at the UI they were trying to escape. That is collie#31 all over again.
//
...
describe("the /auth reservation agrees across the service worker and the bridge", () => {
  for (const { url, reserved } of CASES) {
    test(`${url} → ${reserved ? "reserved" : "Sightr's"}`, () => {
