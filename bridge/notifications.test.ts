...
describe("NotificationCoordinator — debounce", () => {
  ...
  test("uses Herdr's live agent name when the pane has one", () => {
    const { clock, sink, coord } = setup();
    coord.onTransition(
      { ...agent("p1", "blocked"), agentName: "planner-7dba2785" },
      "working",
      "blocked",
    );
    clock.fireAll();
    expect(sink.last?.title).toBe("planner-7dba2785 needs you");
  });
});
...
describe("NotificationCoordinator — coalescing", () => {
  test("two outstanding agents collapse into one digest that buzzes", () => {
    ...
    coord.onTransition(agentNamed("p2", "grok", "blocked"), "working", "blocked");
    clock.fireAll();
  ...
  test("a mixed blocked+done herd reads as 'need attention'", () => {
    const { clock, sink, coord } = setup();
    coord.onTransition(agentNamed("p1", "claude", "blocked"), "working", "blocked");
    coord.onTransition(agentNamed("p2", "grok", "done"), "working", "done");
    clock.fireAll();
  ...
  test("resolving one of two falls back to the named single, silently", () => {
    ...
    coord.onTransition(agentNamed("p2", "grok", "blocked"), "working", "blocked");
    clock.fireAll();
    coord.onTransition(agentNamed("p2", "grok", "idle"), "blocked", "idle"); // grok handled
    expect(sink.last).toEqual({
      title: "claude needs you",
      body: "demo · /home/you/demo",
      paneId: "p1",
...
describe("makeNotifySink", () => {
  ...
  test("render maps the summary onto a single herd-tagged push", () => {
    ...
    makeNotifySink(push, "sightr:herd").render(summary);
    expect(push.sent).toEqual([
      { title: "claude needs you", body: "demo · /home/you/demo", tag: "sightr:herd", paneId: "p1", renotify: true },
    ]);
  });

  test("clear maps to a clear push on the herd tag", () => {
    const push = new RecordingPush();
    makeNotifySink(push, "sightr:herd").clear();
    expect(push.sent).toEqual([{ type: "clear", tag: "sightr:herd" }]);
  });

  test("nothing mutes push: render and clear always reach the sender", () => {
    const push = new RecordingPush();
    const sink = makeNotifySink(push, "sightr:herd");
    sink.render(summary);
    sink.clear();
    expect(push.sent).toEqual([
      { title: "claude needs you", body: "demo · /home/you/demo", tag: "sightr:herd", paneId: "p1", renotify: true },
      { type: "clear", tag: "sightr:herd" },
    ]);
  });
});
