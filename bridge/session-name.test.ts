...
describe("extractClaudeSessionName — named sessions", () => {
  test("reads the name embedded in the rule above the ❯ prompt (fixture)", () => {
    // claude--working.txt was captured with a renamed session ("sighter upgrades", the pre-1.0 name).
    expect(extractClaudeSessionName(fixture("claude--working"))).toBe("sighter upgrades");
  });
