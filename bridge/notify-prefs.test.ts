...
// is verified through a throwaway temp state dir (mirrors push.test.ts).

const dirs: string[] = [];
async function tempCfg() {
  const stateDir = await mkdtemp(join(tmpdir(), "sightr-notify-prefs-"));
  dirs.push(stateDir);
...
describe("coerceNotifyPrefs", () => {
  test("fills missing / non-boolean keys from defaults", () => {
    ...
    // Non-booleans fall back to the default.
