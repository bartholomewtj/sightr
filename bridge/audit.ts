...
// Append-only audit trail of write-level actions (one API call types into a live terminal, so
 ...
 * (ts, action, paneId, device) are untouched either way — the point is to keep answering
 ...
 * (ts, action, paneId?, device?, detail) so lines are grep/diff-friendly; optional attribution
 * is omitted (not null) when absent. Pure — `now` (epoch ms) is injected so tests are deterministic.
 */
export function formatAuditLine(
  ...
  if (entry.paneId != null) line.paneId = entry.paneId;
  if (entry.device != null) line.device = entry.device;
  line.detail = sanitize(entry.detail ?? {}, content);
  return JSON.stringify(line);
}

/** Permissions for the audit log file (owner-only, 0600). */
export const AUDIT_FILE_MODE = 0o600;

/** Default audit log filename inside the state directory. */
export const AUDIT_FILENAME = "audit.log";

/**
 * Cap on the live `audit.log` before it rotates (5 MiB).
 *
 * Anyone who can reach the listener can make this file grow, so its growth must be bounded here
 * rather than by who is calling. Not an env var and not a config key: a knob whose only wrong
 * setting is "unbounded" is not a choice worth offering.
 */
...
export function fsAuditFileIo(): AuditFileIo {
  return {
    ...
    append: (path, line) => appendFile(path, line, { mode: AUDIT_FILE_MODE }),
  };
}
 ...
 *
 * Appends are chained so the size is tracked in memory (seeded by one `stat`, incremented per write)
 * rather than `stat`ed per line.
 *
...
export class AuditLog {
  ...
  constructor(
    ...
    opts: {
      ...
      /**
       * Fields stamped on every entry this instance records. See {@link AuditLog.scoped}.
       */
  ...
  /**
   * A view of this log that stamps `defaults` onto every entry. Used to stamp the attributed device
   * once per write request without handlers needing a device parameter they could forget.
   */
...
/** A no-op audit log that writes nothing and creates no file. */
export const SILENT_AUDIT: AuditLog = new AuditLog(() => {});

/** Parse SIGHTR_AUDIT_CONTENT into "preview" or "none", warning on invalid values. */
export function parseAuditContent(raw: string | undefined): AuditContent {
  if (raw === undefined || raw.trim() === "") return "preview";
  const v = raw.trim().toLowerCase();
  if (v === "none") return "none";
  if (v === "preview") return "preview";
  console.warn(
    `[config] SIGHTR_AUDIT_CONTENT="${raw}" is not valid (expected "preview" or "none") — using default preview`,
  );
  return "preview";
}

/** Create the process audit log. Returns SILENT_AUDIT when !enabled. */
export function createAuditLog(opts: {
  stateDir: string;
  enabled: boolean;
  content: AuditContent;
}): AuditLog {
  if (!opts.enabled) return SILENT_AUDIT;
  const filePath = join(opts.stateDir, AUDIT_FILENAME);
  return new AuditLog(fileAuditAppender(filePath), { content: opts.content });
}
