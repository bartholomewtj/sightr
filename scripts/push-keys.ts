...
//   contrib\windows\sightr-ctl.ps1 push-keys ["mailto:you@example.com"]
...
export function mergeEnv(text: string, vars: Record<string, string>): string {
  ...
  if (appended.length > 0) {
    ...
    lines.push("", "# --- Web Push (VAPID) — written by `sightr-ctl.ps1 push-keys` ---", ...appended);
  }
 ...
 *
 * The character set is an ALLOWLIST, and that is not decoration. This value is written into a .env
 * that more than one reader parses (scripts/ctl/env.ts and the bridge's own loader), and a value
 * holding shell metacharacters, quotes or a newline is exactly the kind that two parsers can read
 * differently — or that a shell sourcing the file would execute. A config file that means two
 * things is a bug regardless of who wrote it. The set below covers every mailto/https subject anyone
 * actually uses.
 */
export function validateSubject(subject: string): string {
  ...
  // No `&` and no `~` either — both read differently to a shell than to a plain KEY=value parser.
...
if (import.meta.main) {
  ...
  const already = VAPID_KEYS.filter((k) => k !== "SIGHTR_VAPID_SUBJECT").every(
  ...
  if (already && !force && !subjectOnly) {
    console.error(
      `✗ push keys are already configured in ${envPath}\n` +
        "  Replacing them invalidates EVERY existing subscription: each device must open Sightr and\n" +
        "  re-enable notifications, and until it does it will silently receive nothing.\n" +
        "  If that's what you want: `contrib\\windows\\sightr-ctl.ps1 push-keys --force`.\n" +
  ...
  if (!subjectOnly) {
    ...
    vars.SIGHTR_VAPID_PUBLIC = keys.publicKey;
    vars.SIGHTR_VAPID_PRIVATE = keys.privateKey;
  }
  const effectiveSubject = subject ?? readEnvVar(text, "SIGHTR_VAPID_SUBJECT");
  if (effectiveSubject !== undefined) vars.SIGHTR_VAPID_SUBJECT = effectiveSubject;
  ...
  if (subjectOnly) {
    console.log(`✓ updated SIGHTR_VAPID_SUBJECT in ${envPath} — the keys are untouched`);
  } else {
    console.log(`✓ wrote SIGHTR_VAPID_PUBLIC / _PRIVATE to ${envPath} (mode 600)`);
  ...
  console.log("\nNext:");
  console.log("  1. herdr plugin action invoke restart --plugin herdr.sightr");
  console.log("  2. On your phone: open Sightr → Settings → enable notifications");
  console.log("  3. contrib\\windows\\sightr-ctl.ps1 push-test  (or the `push-test` Herdr action)");
}
