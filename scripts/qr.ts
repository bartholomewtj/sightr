...
// typing a MagicDNS name on a phone keyboard. Called only by `sightr-ctl.ps1 qr`, which decides WHICH
// url (the tailnet front door, or SIGHTR_PUBLIC_URL behind an operator-run proxy, README.md → Configuration) and whether it's worth one.
//
...
// it IS the polarity fix, so stripping it hands `sightr-ctl.ps1 qr | less -R` (or any capture you then
