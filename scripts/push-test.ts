...
//
// Needs the same env the bridge runs with (SIGHTR_VAPID_*). Run it via
//   contrib\windows\sightr-ctl.ps1 push-test ["title"] ["body"] ["paneId"]
...
const [title = "Sightr test 🐕", body = "Push works — tap to open Sightr", paneId = "test"] =
...
if (!push.enabled) {
  console.error(
    "✗ push is disabled — SIGHTR_VAPID_PUBLIC/PRIVATE aren't set (or web-push isn't installed).\n" +
      "  Run via `contrib\\windows\\sightr-ctl.ps1 push-test` so the plugin .env is loaded first.",
  );
  process.exit(1);
}
...
if (count === 0) {
  console.error(
    `✗ no subscribed devices in ${subsFile}\n` +
      "  Open the Sightr PWA on your phone and enable notifications (Settings → push), then retry.",
  );
  process.exit(1);
}
...
console.log(
  `✓ sent "${title}" to ${count} device(s). Check your phone` +
    " (and `journalctl --user -u sightr` for any per-endpoint send errors).",
);
