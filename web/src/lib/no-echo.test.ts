...
// collie#103: the screen where the reply guard's evidence can never arrive. The detector decides nothing —
...
describe("detectNoEchoPrompt", () => {
  it("recognises the prompts sudo, ssh, ssh-add and gpg print", () => {
    expect(detectNoEchoPrompt(screen("$ sudo systemctl restart sightr\n[sudo] password for altan:")))
  ...
  it("does not claim a screen that merely mentions a password", () => {
    expect(detectNoEchoPrompt(screen("I'll run sudo systemctl restart sightr for you"))).toBeNull();
