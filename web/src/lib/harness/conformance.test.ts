...
//
// The FOREIGN cohort is the other adapters' corpora (grok and agy). Until it existed this leg was
// vacuous (`foreignFixtures: []`), so "an adapter must stay raw on another harness's screens" was a
// documented promise nothing checked. Claude's grammars return raw-only on those captures today,
// and the mirror-image assertions live in harness/grok.test.ts
// and harness/agy/agy.test.ts.
...
const NEUTRAL = [
  ...
  // captured complete and half-arrived. Still an input box with a draft in it; pinned below (collie#110).
...
describeAdapterConformance(claudeAdapter, {
  ownFixtures,
  foreignFixtures: [...allGrokFixtures, ...allAgyFixtures, ...allCursorFixtures, ...allPiFixtures],
  neutralFixtures,
});
