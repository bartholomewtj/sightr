...
const foreignFixtures = allFixtures.filter(
  (f) => !f.startsWith("agy--") && f.endsWith(".txt"),
);
...
describe("agyAdapter unit & footer safety", () => {
  ...
  it("rejects a box separated from its prompt by more than the shared blank budget", () => {
    const raw = [RULE, ...Array.from({ length: 101 }, () => ""), "> ", RULE, "? for shortcuts"].join("\n");
    const lines = splitLines(parseAnsi(raw));
    const texts = lines.map((line) => line.segments.map((segment) => segment.text).join(""));
    expect(locateInputBox(texts, texts.length)).toBeNull();
  });
  ...
  // indistinguishable from a live composer, so an unanchored `>` must never claim one: Sightr would
