...
export interface MenuModel {
  ...
  signature: string;
  /**
   * Literal on-screen text bound to the first keystroke (`expected_prompt`). Must sit at the buffer
   * tail and stay under 8192 characters (bridge/pane-write-routes.ts). A tall picker (Grok plan
   * review) signs the whole preview for freshness and binds only the composer+footer slice; a small
   * picker can set this equal to `signature`.
   */
  region: string;
}
