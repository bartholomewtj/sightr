...
export interface PromptOption {
   ...
   * ask card sends `["Tab", "2"]` but the row is still "option 2". */
  keyLabel?: string;
}

/** Why the inline free-text row exists. Absent means Claude's plan-approval input. */
 ...
 *   - `text` — what the box holds. Empty (the row shows its placeholder) is the only state Sightr
...
export interface PromptFeedback {
  ...
  text: string;
  /** Absent means `plan-change` (Claude); set explicitly for another harness's custom row. */
...
export function promptsSameIdentity(a: PromptModel, b: PromptModel): boolean {
  return (
    ...
    // The row's key and purpose, not its state: a feedback row that appeared, vanished, renumbered,
    // or changed purpose is a different dialog, and the flow's remaining keystrokes would be aimed
    // at the wrong row.
