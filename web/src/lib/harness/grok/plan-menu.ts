...
import { composerStatus, isComposerTop, lineText, } from "./markers";
...
export function detectPlanMenuRegion(lines: StyledLine[]): PlanMenuRegion | null {
  ...
  // Bind the composer box + hint footer, not the plan.md preview. A real plan fills the viewport
  // and the full signature blows past the bridge's 8192-char expected_prompt cap, so Approve
  // 400s and never types. The client still compares `signature` (the whole preview) so a swapped
  // plan is refused; the bridge only needs a tail token that vanishes when review ends.
  let bindStart = composerBottom;
  for (let i = composerBottom - 1; i >= 0 && composerBottom - i <= 100; i--) {
    if (isComposerTop(texts[i]!)) {
      bindStart = i;
      break;
    }
  }
  const region = regionSignature(texts, bindStart, fi + 1);
