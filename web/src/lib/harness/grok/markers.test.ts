...
describe("isComposerHint", () => {
  it("accepts the captured shortcut bar", () => {
    ...
    expect(isComposerHint("  Shift+Tab:mode  │  Ctrl+x:shortcuts")).toBe(true);
    ...
    expect(
      isComposerHint("  Shift+Tab:mode  │  Esc:cancel  │  Ctrl+b:send to bg  │  Ctrl+x:shortcuts"),
    ).toBe(true);
  ...
  it("rejects ordinary transcript, chips, and colon-shaped noise", () => {
    ...
    expect(isComposerHint("  [stable]  ")).toBe(false);
    expect(isComposerHint("Grok Build  1.0.13 [stable]")).toBe(false);
  });
});
...
describe("isStatusChipRow", () => {
  ...
  it("accepts the captured Grok Build <semver> [stable] chip", () => {
    expect(isStatusChipRow("                                             Grok Build  1.0.13 [stable]")).toBe(
      true,
    );
  ...
  it("rejects every other bracket run — uncaptured tags are torn transcript, not chrome", () => {
    ...
    expect(isStatusChipRow("Grok Build  1.0.13 [preview]")).toBe(false);
    expect(isStatusChipRow("Grok Build  1.0.13 [waiting]")).toBe(false);
    expect(isStatusChipRow("started Grok Build  1.0.13 [stable]")).toBe(false);
    expect(isStatusChipRow("Grok Build  1.0.13 [stable] extra")).toBe(false);
...
describe("rstrip", () => {
  ...
  // Herdr's text snapshot may draw a scrollbar as █. Sightr never parses that snapshot — only
...
describe("live-shaped SGR (probed 2026-08-21 against herdr pane.read format:ansi)", () => {
  ...
  function box(draft: string, status: string, prompt = "❯", hint = "Ctrl+.:shortcuts"): string {
    const grey = (t: string) => `${ESC}[38;2;80;80;88m${t}${ESC}[0m`;
    const fg = (t: string) => `${ESC}[38;2;200;200;200m${t}${ESC}[0m`;
    const top = grey(`  ╭${"─".repeat(48)}╮`);
    const inner = grey("  │") + fg(` ${prompt} ${draft}`) + grey(`${" ".repeat(Math.max(0, 40 - draft.length))}│`);
    const bottom = grey(`  ╰${"─".repeat(20)} `) + fg(status) + grey(" ─╯");
    return [top, inner, bottom, "", `  Shift+Tab:mode  │  ${hint}`, ""].join("\n");
  }
  ...
  it("the live ASCII > prompt is the same shape — separate SGR runs still classify", () => {
    const lines = splitLines(parseAnsi(box("hello", "Grok 4.6 (high) · auto", ">", "Ctrl+x:shortcuts")));
    const prompt = lines[1]!.segments.map((s) => s.text).join("");
    expect(composerPromptText(prompt)?.trim()).toBe("hello");
    expect(hasComposer(lines)).toBe(true);
    expect(isComposerHint("  Shift+Tab:mode  │  Ctrl+x:shortcuts")).toBe(true);
