...
// the adapter under test plus three fixture cohorts, so a future adapter gets
...
import { lineText, parseLines, type Block, type StyledLine } from "../blocks";
...
function loadLines(name: string): StyledLine[] {
  return parseLines(readFileSync(join(PANES_DIR, name), "utf8"));
}
 ...
 */
function isValidHerdrKey(key: string): boolean {
...
/** Max entries and characters in an adapter's submitKeys, matching the bridge wire caps. */
const MAX_SUBMIT_KEYS = 4;
const MAX_SUBMIT_KEY_CHARS = 32;

/** Whether an adapter's submit keys satisfy the bridge wire caps and Herdr key grammar. */
function isValidSubmitKeys(keys: string[]): boolean {
  return keys.length > 0 && keys.length <= MAX_SUBMIT_KEYS &&
    keys.every((key) => key.length <= MAX_SUBMIT_KEY_CHARS && isValidHerdrKey(key));
}
...
function emittableKeys(block: Block): string[] | null {
  switch (block.kind) {
    ...
    case "preview-select": {
      // actions.ts's recipe: a digit moves the pointer, Enter selects, `n` opens the note
    ...
    case "multi-select":
      // Claude: a digit toggles each option (and the "Chat about this" escape), Up/Down move the
      // pointer, Enter activates it. Grok `tab-space-enter`: Tab walks, Space toggles, Enter
  ...
  describe(`HarnessAdapter conformance — ${adapter.agent}`, () => {
    ...
    describe("composerPrompt agrees with composerReady", () => {
      ...
      if (!adapter.composerPrompt || !adapter.composerReady) {
        ...
        // but a permanent refusal the user cannot act on. An earlier adapter shipped one row of margin here (its
    ...
    describe("menu blocks (the generic modal contract)", () => {
      ...
      for (const name of menuFixtures) {
        ...
        // say no here — otherwise the reply pre-flight (lib/actions.ts) would type the user's
        ...
        for (const name of kindFixtures) {
          it(`${name}: the ${kind} model signs itself (signature + bound region non-empty)`, () => {
            for (const model of modelsOf(adapter, name, kind)) {
              ...
              // bridge/pane-write-routes.ts MAX_EXPECTED_PROMPT_CHARS. A region over the cap 400s
              // as "bad expected_prompt" and the tap never types — Grok plan review hit this when
              // the bind was the whole plan.md preview.
              expect(
                contract.region(model).length,
                `${name} binds its ${kind} to ${contract.region(model).length} chars (cap ${MAX_REPLY_BYTES})`,
              ).toBeLessThanOrEqual(MAX_REPLY_BYTES);
            }
          });
    ...
    describe("submitKeys grammar", () => {
      const keys = adapter.submitKeys;
      if (!keys) it.todo("adapter names no submit keys (the bridge's default applies)");
      else it("is a sendable, capped key list", () => expect(isValidSubmitKeys(keys)).toBe(true));
    });
