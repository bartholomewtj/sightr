...
export interface WizardOption {
  ...
  escape: boolean;
  /** What the option's badge shows when `keys[0]` is not the identifying key — e.g. Pi's ask options
   *  send Up/Down arrows then Enter, but the row is still option 1, 2, etc. */
  keyLabel?: string;
}
