 ...
 *    with echoed question/answer rows.
 *
...
export type MultiSelectModel =
  | {
       ...
       * clears it, so comparators ignore this field.
       */
      parked?: boolean;
      /**
  ...
  | {
      ...
      incomplete: boolean;
      /**
       * The echoed answers Claude lists above "Ready to submit your answers?" (`● <question>` /
       * `→ Cheese, Pepperoni`), same shape as the wizard review's. Empty when none parse — the card
       * is then confirm-only; labels are never invented.
       */
...
export function multiSelectEquals(a: MultiSelectModel, b: MultiSelectModel): boolean {
  ...
  if (a.phase === "review" && b.phase === "review") {
    // answers are covered by signature (stepper-to-tail span)
