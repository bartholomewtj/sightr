...
describe("detectMenuRegion — what it must decline", () => {
  ...
  it("declines a bottom row that is a statusline with one hint spliced into it", () => {
    // A right-flushed statusline sharing the row with a hint: the column gap is the tell. Reading it
    // as a footer yields a single action whose label swallows the statusline.
    const screen = [
      BOX_RULE,
      "Do you want to proceed?",
      "❯ 1. Yes",
      "  2. No",
      `esc to cancel${" ".repeat(40)}Some Model 1.0 · low`,
    ].join("\n");
    expect(detectMenu(lines(screen))).toBeNull();
  });

  it("declines a footer carrying a segment that is not a hint at all", () => {
    const screen = [BOX_RULE, "Pick one", "❯ a", "  b", "Enter to go on · Esc to cancel · v1.2.3"].join(
      "\n",
    );
    expect(detectMenu(lines(screen))).toBeNull();
  });
