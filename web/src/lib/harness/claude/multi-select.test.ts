...
describe("detectMultiSelect — review phase", () => {
  it("detects the answered review screen and parsed answers", () => {
    ...
    expect(model!.answers).toEqual([
      { question: "Which pizza toppings do you want?", answer: "Cheese, Pepperoni" },
    ]);
  });

  it("detects an incomplete review with warning (⚠)", () => {
    const buf = [
      "←  ☐ Toppings  ✔ Submit  →",
      "",
      "Review your answers",
  ...
  it("an empty review parse (no echo rows and no ⚠) stays confirm-only", () => {
    const buf = [
      "←  ☒ Toppings  ✔ Submit  →",
      "",
      "Review your answers",
