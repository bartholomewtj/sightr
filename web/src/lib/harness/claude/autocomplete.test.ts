...
describe("detectAutocompleteRegion", () => {
  ...
  it("declines a path-shaped statusline row", () => {
    ...
    expect(detectAutocompleteRegion(screen("/re", ["  /home/user/project    main ✓"]))).toBeNull();
  });
});
...
describe("the input box survives the popup", () => {
  ...
  it("reports no statusline — Claude paints the popup in its place", () => {
    ...
    // Before the peel this returned the popup's three rows and painted them as the status strip.
    expect(extractStatusLines(load(SHORT))).toEqual([]);
  });
