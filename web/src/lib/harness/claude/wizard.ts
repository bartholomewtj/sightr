...
// own tail anchor. The round-trip model is INCREMENTAL: Sighter mirrors the current step, each tap
...
function detectQuestionPhase(
  ...
  return {
    model: {
      ...
      question,
      options,
      signature: regionSignature(texts, stepperIdx, fi + 1),
    },
...
function detectReviewPhase(lines: StyledLine[], texts: string[], fi: number): WizardRegion | null {
  ...
  const { answers, incomplete } = parseReviewBody(texts, stepperIdx + 1, ri);
...
/** The review screen's body between the stepper (exclusive) and the "Ready to submit" prompt
 *  (exclusive): echoed "● <question>" / "→ <answer>" pairs (wrapped lines continue the open field),
 *  the "Review your answers" heading (skipped) and the ⚠ not-answered warning (→ incomplete).
 *  Shared by the wizard and multi-select review detectors so the echo grammar lives in one place. */
export function parseReviewBody(
  texts: string[],
  from: number,
  to: number,
): { answers: WizardAnswer[]; incomplete: boolean } {
