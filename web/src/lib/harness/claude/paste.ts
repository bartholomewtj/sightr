...
//
// The collie#34 guard (lib/reply-action.ts) only presses the submit key once it can SEE the text it typed
...
//
// Facts it is built on (live-probed 2026-08-06, sightr-demo sandbox, Claude Code current):
...
//     the row is the last thing that arrived — re-probed 2026-08-17 (sightr-demo, pane `w6:p1`,
 ...
 *     somewhere inside it (collie#110). Rules 2 and 4 both pass a PARTIALLY arrived send: `Σ M ≤ S` is
 ...
 *     "later chunks are still missing". Live-probed 2026-08-17 (sightr-demo, pane `w6:p1`):
...
export function pasteCarriesSend(sent: string, draft: string): boolean {
  ...
  // missing (collie#110). No length exemption here: `extractInputDraft` trims the row, so a trailing scrap
