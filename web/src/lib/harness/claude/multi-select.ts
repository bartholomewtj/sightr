...
//     `❯ 1. Submit answers` / `2. Cancel`, with the same single-question stepper and echoed
//     `● question / → answers` rows. Digit 1 submits, 2 cancels; a `⚠ …not answered all questions`
//     line means `incomplete`.
//
...
import { parseReviewBody, parseStepperLine } from "./wizard";
...
function detectReviewPhase(
  ...
  // `incomplete` and echoed answers between the stepper and the prompt.
