...
describe("isHorizontalRule", () => {
  ...
  it("rejects ordinary text, option rows, and ASCII rules", () => {
    ...
    expect(isHorizontalRule("── sightr upgrades ──")).toBe(false); // embedded label ⇒ not a pure rule
  });
});
...
describe("isBoxBorder", () => {
  it("matches a long rule run even with an embedded label (input-box top border)", () => {
    ...
    expect(isBoxBorder("─".repeat(30) + " sightr upgrades " + "──")).toBe(true);
  });
  ...
  // collie#76: a bare border's old test required 20+ consecutive rule glyphs ANYWHERE on the line.
  ...
  // at a normal pane width — this is collie#76's second failure mode. Observed real capture: flanks
  // 5/2 at 43 columns.
