...
import { draftCarriesSend } from "../../actions";
...
// Same shape as boxBuffer, but with a caller-chosen border WIDTH — for pinning collie#76's
...
describe("extractStatusLines — recovers the stripped statusline run", () => {
  ...
  it("with no popup, the statusline walk below the box still finds a real statusline", () => {
    const rows = statusText(fixtureLines("claude--fresh-idle.txt"));
    expect(rows.length).toBe(2);
    expect(rows[0]).toContain("fixture-sandbox");
    expect(rows[1]).toContain("← for agents");
  ...
  it("a box whose draft starts with / but with a genuine statusline below it still reads the statusline", () => {
    const lines = boxWithStatusRows("❯\u00A0/command", [
      "  [Opus 4.8] · ctx:3% · main · 32k tokens",
...
describe("extractInputDraft — recovers a stranded prompt-line draft", () => {
  ...
  // collie#76 fix: the wrapped-draft scan used to be bounded by MAX_DRAFT_LINES (12), so a draft long
  ...
  // actions's draftCarriesSend needs to verify the send actually landed.
  ...
  // collie#76 (finding 2, defense-in-depth): the draft-walk cap (100) exists so a stray line that merely
...
// collie#76, first failure mode: isBoxBorder used to require 20+ consecutive rule glyphs, which is
...
describe("real corpus — pinned so any change to the walk shows up as a diff", () => {
  ...
  const PINNED: { fixture: string; statusRows: number; draft: string | null; stripped: number }[] = [
    ...
    // The split shape (collie#110): a token plus the literal tail beside it, captured complete and
