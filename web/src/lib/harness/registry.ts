...
// adding a further verified agent is a one-line change to ADAPTERS. The list holds six harnesses today:
// claude, which lifts every block kind; grok, which is Tier 1 chrome plus Tier-2 probed permission /
// ask / plan lifts; agy/antigravity, the Antigravity CLI, which lifts boxed-composer chrome plus
// permission / plan / trust / select menu prompts and registers under two exact strings; cursor,
// which is Tier 1 chrome plus Tier-2 command-approval and workspace-trust lifts; and pi, Tier-2
// ask lift only (prompt-select / wizard), exact `pi`, one-shot replies.
// Adapters register by their EXACT agent string only — prefix-matching here
// was the collie#99 reject; variant tolerance for slash catalogs belongs in canonicalAgent. `hasBlockGrammar` replaces the old grammar/agents predicate:
...
const ADAPTERS: Record<string, HarnessAdapter> = Object.fromEntries(
  [claudeAdapter, grokAdapter, agyAdapter, antigravityAdapter, cursorAdapter, piAdapter].map((a) => [
    a.agent,
    a,
  ]),
);
...
export const ADAPTER_AGENTS: readonly string[] = Object.keys(ADAPTERS);
