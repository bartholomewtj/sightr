...
export async function sendBoundKeys(
  ...
  try {
    const res =
      region === undefined
        ? await sendKeys(target.paneId, keys)
        : await sendKeys(target.paneId, keys, region);
...
/** A pane read the view can adopt as-is. */
export interface PaneSnapshot { text: string; revision: number }

export type SettleOutcome =
  | { status: "settled"; snapshot: PaneSnapshot }
  | { status: "timeout" };

/** ~5.6s at POLL_DELAY_MS. Long enough for a Submit that closes the dialog; bounded so a key the
 *  TUI ignores never leaves a card locked. */
export const SETTLE_ATTEMPTS = 16;
/** Consecutive no-dialog reads needed before "gone" is believed (a redraw can hide the tail once). */
export const SETTLE_GONE_READS = 2;

/**
 * Wait for a fresh snapshot before unlocking after a successful send (#368).
 *
 * sendKeys resolving only means the bytes reached Herdr. The TUI has not necessarily redrawn yet,
 * and reading immediately can observe the pre-tap screen — unlocking against the old snapshot
 * (e.g. unchecked checkbox, old question).
 *
 * This helper re-reads the pane in a bounded loop until:
 *   1. A different dialog of this kind is derived (fresh != tapped per the kind's comparator), or
 *   2. No dialog of this kind is seen for TWO consecutive reads (a single null can be a redraw flicker).
 * If the screen never changes within SETTLE_ATTEMPTS, it returns timeout so the card still unlocks.
 * It never sends a keystroke.
 */
export async function awaitDialogSettled<K extends DialogKind>(
  target: DialogTarget<K> & { sleep?: Sleep },
  compare: Compare = "commits",
): Promise<SettleOutcome> {
  const detect = dialogDetector(target.kind, target.agent);
  const same = DIALOG_CONTRACT[target.kind][compare];
  ...
  for (let attempt = 0; attempt < SETTLE_ATTEMPTS; attempt++) {
    ...
    let fresh;
    try {
      fresh = await fetchPane(target.paneId, target.requestedLines);
    } catch {
      continue;
    }
    const model = detect(parseLines(fresh.text));
    if (model === null) {
      goneStreak++;
      if (goneStreak >= SETTLE_GONE_READS) {
        return {
          status: "settled",
          snapshot: { text: fresh.text, revision: fresh.revision },
        };
      }
      continue;
    }
    goneStreak = 0;
    if (!same(model, target.model)) {
      return {
        status: "settled",
        snapshot: { text: fresh.text, revision: fresh.revision },
      };
