...
//   lastSeenAt   — when you last opened or drove it through Sighter
 ...
 *  A parsed ask card is Needs you regardless of Herdr status (#372). Pi/Cursor
 *  asks with no parsed dialog stay out (#366/#370). */
export function bucketOf(a: AgentView): TriageKey {
  if (a.status === "blocked" || a.dialogPresent === true) return "needs";
...
export function isAttention(a: AgentView): boolean {
  return bucketOf(a) === "needs";
}

/** The status a per-pane dot/badge should draw: blocked when the pane is in Needs you (Herdr blocked OR
 *  a parsed ask card), else Herdr's own status. Routes through bucketOf, so a row and its chip agree. */
export function shownStatus(a: AgentView): AgentStatus {
  return bucketOf(a) === "needs" ? "blocked" : a.status;
}
