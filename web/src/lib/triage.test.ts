...
function agent(
  paneId: string,
  status: AgentStatus,
  ts: { active?: number; seen?: number; dialog?: boolean } = {},
): AgentView {
  return {
    paneId,
    workspaceId: "w0",
    ...
    ...(ts.dialog !== undefined ? { dialogPresent: ts.dialog } : {}),
  };
}
...
describe("a parsed ask card (#372)", () => {
  it("working + dialogPresent -> bucketOf is 'needs', triage puts it in needs, and worstTriage gives 'needs'", () => {
    const a = agent("p", "working", { dialog: true });
    expect(bucketOf(a)).toBe("needs");
    const sections = triage([a]);
    expect(sections.find((s) => s.key === "needs")!.agents).toEqual([a]);
    expect(worstTriage([a, agent("idle", "idle")])).toBe("needs");
  });

  it("done + unseen + dialogPresent -> 'needs', not 'ready'", () => {
    const a = agent("p", "done", { active: 200, seen: 100, dialog: true });
    expect(bucketOf(a)).toBe("needs");
  });

  it("idle + dialogPresent -> 'needs'", () => {
    const a = agent("p", "idle", { dialog: true });
    expect(bucketOf(a)).toBe("needs");
  });

  it("blocked with no dialog -> still 'needs'", () => {
    const a = agent("p", "blocked");
    expect(bucketOf(a)).toBe("needs");
  });

  it("no dialog: idle -> recent, done seen -> recent, done unseen -> ready, working -> working (and with dialogPresent: false)", () => {
    for (const dialog of [undefined, false]) {
      expect(bucketOf(agent("p1", "idle", { dialog }))).toBe("recent");
      expect(bucketOf(agent("p2", "done", { active: 100, seen: 200, dialog }))).toBe("recent");
      expect(bucketOf(agent("p3", "done", { active: 200, seen: 100, dialog }))).toBe("ready");
      expect(bucketOf(agent("p4", "working", { dialog }))).toBe("working");
    }
  });

  it("shownStatus: working+dialog -> blocked, working -> working, done -> done, blocked -> blocked", () => {
    expect(shownStatus(agent("p1", "working", { dialog: true }))).toBe("blocked");
    expect(shownStatus(agent("p2", "working"))).toBe("working");
    expect(shownStatus(agent("p3", "done"))).toBe("done");
    expect(shownStatus(agent("p4", "blocked"))).toBe("blocked");
  });

  it("isAttention: working+dialog true, working false, blocked true", () => {
    expect(isAttention(agent("p1", "working", { dialog: true }))).toBe(true);
    expect(isAttention(agent("p2", "working"))).toBe(false);
    expect(isAttention(agent("p3", "blocked"))).toBe(true);
  });
});
