...
//
// every access is behind a `typeof localStorage` check
 ...
 * anywhere else — component state, a second module — would re-open collie#103 in RAM, where nothing is
...
// "default" is the herdr session name the key used to carry; kept so drafts saved before the
// multi-session removal still load.
function keyFor(paneId: string): string {
  return `${PREFIX}default:${paneId}`;
}
...
function loadStored(paneId: string): DraftEntry | null {
  const store = storage();
  if (!store) return null;
  try {
    const entry = parse(store.getItem(keyFor(paneId)));
    if (entry === null) return null;
    if (Date.now() - entry.at > MAX_AGE_MS) {
      store.removeItem(keyFor(paneId));
      return null;
    }
    return entry;
  } catch {
 ...
 */
export function loadDraft(paneId: string): string | null {
  prunedOnce();
  const cached = memory.get(keyFor(paneId)) ?? null;
  const stored = loadStored(paneId);
 ...
 */
export function saveDraft(paneId: string, text: string): void {
  prunedOnce();
  if (text.trim() === "") {
    clearDraft(paneId);
    return;
