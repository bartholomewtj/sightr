...
// can go stale for benign reasons too — e.g. a backgrounded tab pausing polling — where nobody is
 ...
 * from lib/api.ts at the same fetch interception point that captures X-Sightr-Build, so the anchor
 ...
 * bare wall-clock anchor going stale (which happens for benign reasons too, e.g. a backgrounded tab
 * pausing polling, with nobody connecting and no red UI showing — that must NOT latch).
 */
...
// A phone backgrounds Sightr (screen off, app switch) far more than it truly disconnects; timers
