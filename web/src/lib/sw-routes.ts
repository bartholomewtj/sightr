 ...
 * serves at a path Sightr doesn't own. A proxy that authenticates devices ahead of the bridge
 * (an operator-run reverse proxy, README.md → Configuration) has a sign-in or enrolment page, and before this list existed there was no
 ...
 *
 * `/auth/` is therefore RESERVED. Sightr routes nothing there, precaches nothing there, and will
...
/** The live snapshot stream; callers must never route this through a cache. */
...
export const NAVIGATION_NETWORK_ONLY = [
  // `/api/` includes `/api/events`: SSE must always reach the bridge; do not let a runtime/cache
