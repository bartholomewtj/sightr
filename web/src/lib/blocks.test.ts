...
import { lineText, presentBlocks, splitLines, type StyledLine } from "./blocks";
...
describe("buildBlocks — Claude grammars (ctx.agent === 'claude')", () => {
  ...
  it("keeps the plan fixture's long dashed separators as raw rows", () => {
  ...
  // "No adapter", not "non-Claude": grok is a non-Claude agent that DOES have one. What gates these
  // grammars is the registry lookup, not the string "claude".
  it("leaves an agent with no adapter as a single untouched raw block (conservative gating)", () => {
    ...
    expect(blocks).toHaveLength(1);
    expect(blocks[0]!.kind).toBe("raw");
    expect(blocks.some((b) => b.kind === "prompt-select")).toBe(false); // NO menu lifting for pi
  ...
  // into a prompt-select stays a single raw block for a pi agent (above) AND for an unknown/absent
