...
// the sign-in page is invisible, too wide and Sightr's own deep links stop resolving offline. The
...
describe("service-worker navigation passthrough", () => {
  ...
  it("does not leak a Sightr route that merely carries a query string", () => {
    expect(isNetworkOnlyNavigation("/authors?x=1")).toBe(false);
    expect(isNetworkOnlyNavigation("/?s=sightr-demo")).toBe(false);
    expect(isNetworkOnlyNavigation("/pane/w1:p1?s=demo")).toBe(false);
  });

  it("still owns every Sightr route, so deep links keep resolving offline", () => {
    for (const path of [
      "/",
      "/settings",
      "/pane/w1:p1",
  ...
  // A route merely STARTING with the reserved word is Sightr's, not the proxy's: `/authors` must not
