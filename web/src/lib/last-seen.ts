...
const SNAPSHOT_PREFIX = "sightr:last-snapshot:";
const PANE_PREFIX = "sightr:last-pane:";
...
export function saveLastSnapshot(snap: SnapshotResponse, at: number = Date.now()): void {
  write(snapshotKey(), JSON.stringify({ at, value: snap }));
}

/** The last snapshot this tab saw, with the time it was fetched. */
export function loadLastSnapshot(): Cached<SnapshotResponse> | null {
  return decodeSnapshot(readRaw(snapshotKey()));
}

/** Write through the mirror a successful `/api/pane/:id` just returned. */
export function saveLastPaneText(paneId: string, text: string, at: number = Date.now()): void {
  write(paneKey(paneId), encodePane(at, text));
  prunePanes();
}

/** The last mirror this tab saw for a pane, with the time it was fetched. */
export function loadLastPaneText(paneId: string): Cached<string> | null {
  return decodePane(readRaw(paneKey(paneId)));
}

/** Forget a pane's mirror — the ADR 0017 path, and the only reason to delete a single entry. */
export function dropLastPaneText(paneId: string): void {
  remove(paneKey(paneId));
}
