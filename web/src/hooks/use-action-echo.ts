...
// is that idea extracted so the keypad and the key-queue Send share ONE state
...
export function useActionEcho(doneMs: number = ECHO_DONE_MS): ActionEcho {
  ...
  // Guards the post-await writes: a dock that unmounts mid-flight (tapping Close, or closing
  ...
  const run = useCallback(
    async (id: string, action: () => Promise<boolean>) => {
      ...
      set(id, "pending");
      // Mark the action on the PRESS, not the ✓: feedback has to be simultaneous with the tap to
      // read as caused by it.
