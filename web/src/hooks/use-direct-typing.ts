...
/** The composer's pane key is the pane id itself; an empty or malformed key fails closed. */
export function parsePaneKey(paneKey: string): { paneId: string } | null {
  if (paneKey === "" || paneKey.includes("\u0000")) return null;
  return { paneId: paneKey };
}
...
interface DirectTypingOptions {
   ...
   *  device, or  Arming survives none of them — see the disarm effect below. */
  suspended: boolean;
...
// Herdr writes.
//
// It does NOT own the entry point. Arming is an explicit NAMED choice — the "Type into terminal"
// row in the composer's + menu (or the same row, explained, in the Display dock).
// DESKTOP MODE HAS A SECOND ENTRY POINT, AND IT IS STILL NAMED. There is no "Type" row on a
// desktop (the + menu hides it); the named choice moved into Settings as the "Typing surface"
...
// window.blur. Nothing survives a reload.
//
...
// is polled, and the poll stops on a backgrounded tab, on a pane going
...
export function useDirectTyping({
  ...
  function activate() {
    ...
    // Focus synchronously while the contextmenu event still carries browser user activation; a
    // deferred focus selects the field but mobile browsers may refuse to open their software
    // keyboard once that activation has expired. The existing callback still runs after React
    // swaps the controlled value so selection lands at the end.
  ...
  // Arming dies with the view it belongs to. A backgrounded tab and a lost bridge
  ...
  useEffect(() => {
    ...
    const uploadImage = uploadImageOption ?? (async (file: File) => {
      ...
      const result = await api.uploadImage(parsed.paneId, file);
