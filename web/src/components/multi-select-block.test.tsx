...
describe("MultiSelectBlock — review screen", () => {
  it("shows the echoed answers above Submit answers, no stepper, and Submit/Cancel routing", async () => {
    const onAction = vi.fn();
    const user = userEvent.setup();
    ...
    expect(screen.getByText("Which pizza toppings do you want?")).toBeInTheDocument();
    const answerNode = screen.getByText("Cheese, Pepperoni");
    expect(answerNode).toBeInTheDocument();

    const submitBtn = screen.getByRole("button", { name: /Submit answers/ });
    expect(answerNode.compareDocumentPosition(submitBtn) & Node.DOCUMENT_POSITION_FOLLOWING).toBeTruthy();
    ...
    expect(onAction).not.toHaveBeenCalledWith(expect.objectContaining({ kind: "nav" }));
  });

  it("surfaces the not-all-answered warning when incomplete, without answer list", () => {
    const buf = [
      "←  ☐ Toppings  ✔ Submit  →",
      "",
      "Review your answers",
    ...
    const { container } = render(<MultiSelectBlock multi={model} onAction={vi.fn()} />);

    expect(screen.getByRole("alert")).toHaveTextContent("You have not answered all questions");
