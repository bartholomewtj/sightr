...
// Row actions for a single tab (⋯ button or right-click) — the SAME structure as the pane sheet (the user asked for
...
export function TabActionsSheet({
  ...
  async function save() {
    ...
    setSaving(true);
    try {
      const res = await api.renameTab(tab.tabId, trimmed);
      if (res.ok) {
  ...
  async function requestClose() {
    ...
    setClosing(true);
    try {
      const res = await api.closeTab(tab.tabId);
      if (res.ok) {
        onClose();
        onClosed(tab.tabId);
      } else {
