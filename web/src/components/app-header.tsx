...
interface AppHeaderProps {
  // Connection state — the inputs that drive the SightrHome dog. The dog gallops on sustained trouble
   ...
   *  Sightr mark, and goes up exactly one level. Native nav-stack rule — the same control, in the
   *  same corner, on every screen below the bottom bar's destinations. */
  onBack?: () => void;
  /** Show the "Sightr" wordmark beside the mark (dashboard + space). Omit inside a pane — the
...
// The single header shell every screen mounts: the sticky, safe-area-aware zinc bar with the Sightr
...
export function AppHeader({
  ...
  return (
    <header className="sticky top-0 z-20 flex items-center gap-2 border-b-2 border-border bg-muted pl-4 pr-2 py-2 [padding-top:calc(env(safe-area-inset-top)_+_0.5rem)]">
      {override ?? (
        <>
          {onBack ? (
            <button
            ...
            <SightrHome trouble={trouble} lost={lost} wordmark={wordmark} />
          )}
