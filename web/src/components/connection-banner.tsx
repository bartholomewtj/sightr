...
interface ConnectionBannerProps {
  ...
  authError: boolean;
  /** When the data on screen was last actually fetched, if it can be dated. Red copy only. */
  lastSeenAt?: number;
}
...
// the route, the app connection surface) that is the app's entire connection UI — the header
...
// Herdr is the outage, otherwise onLine decides between a true offline drop and an unreachable Sightr.
  ...
  const cause =
    probe === "reachable"
      ...
      : probe === "unreachable" && !online
        ? { copy: "Offline — can't reach Sightr", Icon: WifiOff }
        : { copy: "Can't reach Sightr", Icon: TriangleAlert };
  const copy =
