...
// visible, so the mirror's dark colour space (MIRROR_SPACE, archive/.adr/0002-color-pipeline.md) never enters:
// the grammar parsed the whole popup, there is nothing left that only the raw text could say. That also
// drops the popup's own colours, which is correct here — the highlight they carry is the terminal's cursor
...
export function AutocompleteBlock({ autocomplete }: AutocompleteBlockProps) {
  return (
    <PromptPanel ariaLabel="Completions">
      <OptionGroupCaption>Completions</OptionGroupCaption>
