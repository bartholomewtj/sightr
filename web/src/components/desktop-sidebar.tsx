...
import { homePath, filesPath, settingsPath } from "@/lib/nav";
...
import { sidebarSlot, filesRelFromPath, FILES_FIND_ID, FILES_TREE_ID } from "@/components/desktop-sidebar-slot";
...
export function DesktopSidebar({ data, currentPaneId }: { data: HomeData; currentPaneId?: string }) {
  ...
  const slot = sidebarSlot(pathname);
  ...
  return <aside className="absolute inset-0 flex min-h-0 flex-col overflow-hidden border-r-2 border-border bg-muted">
    ...
    <nav aria-label="Desktop navigation" className="mt-auto shrink-0 border-t-2 border-border p-2">{items.map((item) => <button key={item.label} type="button" aria-current={item.active ? "page" : undefined} onClick={() => navigate(item.to)} className={cn("flex w-full items-center gap-3 rounded px-3 py-2 text-left text-sm", item.active ? "bg-accent font-medium" : "text-muted-foreground hover:bg-accent/50")}><item.icon className="size-4 shrink-0" /><span className="flex min-w-0 flex-1 items-center justify-between gap-2"><span>{item.label}</span></span></button>)}</nav>
