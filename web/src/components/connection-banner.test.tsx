...
describe("ConnectionBanner — the single connection surface", () => {
  ...
  // whole bug (collie#31). If this assertion is ever "fixed" by swapping in a Button, the PWA is bricked
  ...
  it("escalates to a red alert with Retry + Reload once lost, naming Herdr when the bridge answers", async () => {
    ...
    expect(screen.getByRole("button", { name: /retry/i })).toHaveClass("h-11");
    expect(screen.getByRole("button", { name: /reload/i })).toHaveClass("size-11");
  });
  ...
  it("says 'Offline' in red when the probe fails AND the browser reports offline", async () => {
    ...
    renderBanner();
    await act(async () => {});
    expect(screen.getByText("Offline — can't reach Sightr")).toBeInTheDocument();
    expect(screen.getByRole("alert").className).toMatch(/bg-status-blocked/); // offline is always red
  });

  it("says 'Can't reach Sightr' when the probe fails but the browser still reports online", async () => {
    h.lost = true;
    cfg.reachable = false;
    setOnline(true);
    renderBanner();
  ...
  it("dates the red row when the data on screen came from the cache", async () => {
    ...
    await act(async () => {});
    expect(screen.getByRole("alert")).toHaveTextContent(/Can't reach Sightr — last seen \d/);
  });
