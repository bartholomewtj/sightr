...
import { submitWizardKeys, wizardsEqual } from "@/lib/actions";
...
describe("WizardBlock — question step presentation", () => {
  ...
  it("renders keyLabel as the badge when provided instead of keys[0]", () => {
    const model: WizardModel = {
      phase: "question",
      steps: [{ label: "Step 1", answered: false, current: true }],
      question: "Pick one",
      options: [
        { label: "Alpha", keys: ["Down", "Enter"], keyLabel: "2", chosen: false, escape: false },
      ],
      signature: "sig",
    };
    render(<WizardBlock wizard={model} onAction={vi.fn()} />);
...
describe("WizardBlock — review (Submit) step presentation", () => {
  it("lists the echoed answers and offers Submit answers / Cancel", async () => {
    ...
    expect(onAction).not.toHaveBeenCalledWith(["Left"]);
...
describe("WizardBlock — wired tap (component → handler → api)", () => {
  it("tapping an answer runs the guard, sends its single digit, and confirms", async () => {
    ...
    expect(mockSendKeys).toHaveBeenCalledWith("w1:p1", ["3"], model.signature);
  });
