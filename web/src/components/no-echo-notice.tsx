...
interface NoEchoNoticeProps {
   ...
   *  the mode can't be armed at all — a gone pane or a read-only device. */
  onUseType: (() => void) | null;
  onDismiss: () => void;
}

// The one thing collie#103 was missing: a sentence, at the moment of the refusal, that names the screen and
...
// can never change. The reporter of collie#103 gave up and walked to a laptop for three days.
//
    ...
    <div className="mb-2 flex items-start gap-1.5 rounded-md bg-muted/40 px-2.5 py-1.5 text-xs text-muted-foreground">
      ...
      {onUseType !== null && (
        <Button
          variant="ghost"
          size="sm"
          className="h-11 shrink-0 self-center px-2 text-xs font-medium"
          onClick={onUseType}
        >
          Use Type
        </Button>
      ...
      <Button
        variant="ghost"
        size="icon"
        className="size-11 shrink-0 self-start text-muted-foreground"
