...
// the ONLY action here — closing a pane lives in the pane pill's action sheet (with its own
  ...
  return (
    <button
      ...
      className={cn(
        ...
        isAttention(pane) && "border border-status-blocked/40",
        !active && isAttention(pane) && "bg-status-blocked/5",
      )}
    >
      {isShell ? (
