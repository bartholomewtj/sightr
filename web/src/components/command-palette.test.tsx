...
describe("CommandPalette", () => {
  ...
  it("renders one button when a scoped and an unscoped row name the same command", () => {
    ...
    expect(screen.getByText("On pi")).toBeInTheDocument();
  });
  ...
  it("shows the operator's rows INSTEAD of the shipped catalog", () => {
    setup({
      agent: "pi",
      mine: [
        { agent: "pi", command: "/fork-in-herdr", description: "Fork", takesArg: false, argHint: "" },
      ],
    });
  ...
  it("still asks twice before a renamed destructive command", async () => {
    ...
    const props = setup({
      agent: "pi",
      mine: [
        { agent: "pi", command: "/new", description: "Fresh start", takesArg: false, argHint: "" },
      ],
    });
