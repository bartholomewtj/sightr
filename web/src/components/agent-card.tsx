...
import { anchorFrom, contextMenuProps, type MenuPoint } from "@/lib/menu-anchor";
...
export interface RowMoreButtonProps {
  label: string;
  onMore: (at: MenuPoint) => void;
}

export function RowMoreButton({ label, onMore }: RowMoreButtonProps) {
  ...
  return (
    <button
      ...
      onClick={(event) => {
        ...
        onMore(anchorFrom(event.currentTarget));
      }}
      className={cn(
        "flex shrink-0 items-center justify-center rounded-md text-muted-foreground transition-colors hover:bg-muted hover:text-foreground active:scale-95",
        desktop ? "size-9" : "size-11",
      )}
    >
...
interface AgentCardProps {
  ...
  onClick: () => void;
  /**
   * Right-clicking a row opens the caller's actions sheet instead of opening the pane. Unset — the
   * herd list — leaves the row exactly as it was.
   */
  onMenu?: (at: MenuPoint) => void;
  /** Always-visible overflow actions for the Needs you inbox row. */
  onMore?: (at: MenuPoint) => void;
  /**
...
// right-click (contextmenu) opens the caller's pane actions when onMenu is wired; the ⋯ button
// (onMore) is the always-visible way in.
//
...
export function AgentCard({
  ...
  const card = (
      <button
        ...
        className={cn(
          onMore ? "min-w-0 flex-1" : "w-full",
          "text-left transition-transform active:scale-[0.99]",
        // select-none + -webkit-touch-callout:none stop iOS Safari selecting the row text. Only when
        // the menu is wired, so the herd list's markup is untouched.
        onMenu && "select-none [-webkit-touch-callout:none]",
    ...
    >
      ...
      >
        ...
        {isShell ? (
          ...
          <span className="sr-only">{STATUS_LABEL[shown]}</span>
        ) : (
          <StatusBadge status={shown} runningCommand={agent.runningCommand} />
        )}
      </Shell>
      </button>
  );
  ...
  return onMore ? (
    <div className="flex min-w-0 items-center">
      {card}
      <RowMoreButton label="Pane actions" onMore={onMore} />
    </div>
  ) : card;
}
