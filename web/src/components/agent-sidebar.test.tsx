...
describe("ThreadSidebar", () => {
  ...
  it("shows Ready · unseen between Needs you and Working", () => {
    const unseen = { ...idleAgent, paneId: "w3:done", status: "done" as const, lastActiveAt: 20, lastSeenAt: 10 };
    render(<ThreadSidebar agents={[fixtureAgents[0]!, unseen, fixtureAgents[1]!]} currentPaneId="" onSelect={vi.fn()} />);
    expect(screen.getByText("Ready · unseen")).toBeInTheDocument();
    expect(screen.getByText("Ready · unseen").compareDocumentPosition(screen.getByText("Working")) & Node.DOCUMENT_POSITION_FOLLOWING).toBeTruthy();
  });

  it("omits Ready · unseen when no done agent is unseen", () => {
    render(<ThreadSidebar agents={[idleAgent]} currentPaneId="" onSelect={vi.fn()} />);
    expect(screen.queryByText("Ready · unseen")).toBeNull();
  });
  ...
  it("marks the current pane with aria-current='page'", () => {
    ...
    // w2:p1 lives in the "sightr" workspace. The row is titled by where the work IS, not by which
    // agent is doing it — "grok" is carried by the avatar (see paneTitle).
    expect(current).toHaveTextContent("sightr");
    expect(current).not.toHaveTextContent("grok");
  });
